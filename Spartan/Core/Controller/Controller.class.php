<?php
namespace Spartan\Core\Controller;
use Spartan\Core\Controller\Driver\File;
use Spartan\Core\Controller\Driver\Template;
...
abstract class Controller{
    ...
    private function checkCache($tmplTemplateFile) {
        ...
        $tmpCacheFile = APP_PATH.$this->cachePath.md5($tmplTemplateFile).'.php';
        if(!File::instance()->has($tmpCacheFile)){
            return false;
        }elseif (filemtime($tmplTemplateFile)>File::instance()->get($tmpCacheFile,'mtime')){// 模板文件如果有更新则缓存需要更新
            return false;
    ...
    private function checkContentCache($tmplContent) {
        if(File::instance()->has(APP_PATH.$this->cachePath.md5($tmplContent).'.php')){
            return true;
        }else{
            return false;
        }
